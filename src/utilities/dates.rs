...
//! Flexible, dependency-free date parsing shared by the metadata
//! generators (spec A4 of the ssg fixes specification; ssg#586).
//!
//! The news-sitemap generator used to accept only RFC 2822 input:
//! front matter written as `July 1, 2026` or `2026-07-01` fell
//! through and produced the `'day' component could not be parsed`
//! warning spam downstream. This module is the single parsing chain
...
//!    feed consumers already rely on; seconds optional; named zones
//!    from the RFC 2822 obsolete table accepted).
...
/// The formats attempted by [`parse_flexible_date`], in priority
/// order.
///
/// Referenced by [`DateParseError`] so call sites can log exactly
/// what was tried (spec A4: "log which field/format failed").
...
/// (e.g. a feed that passes RFC 2822 strings through verbatim) can
/// branch on this instead of re-formatting.
...
///
/// This is deliberately a plain component struct rather than a
/// wrapper around a date crate — the generators only need
/// formatting, not arithmetic, and the crate keeps its dependency
/// set pinned.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
...
impl FlexibleDate {
    ...
    /// Format as RFC 3339 / ISO 8601 datetime:
    /// `2026-07-01T07:07:07+00:00`.
    ///
    /// UTC is rendered as `+00:00` (not `Z`), matching ssg's copy of
    /// this module. The news sitemap uses [`Self::to_w3c_date`]
    /// instead, which renders UTC as `Z` to keep its existing output
    /// bytes.
    #[must_use]
    ...
    /// `<news:publication_date>`: `2026-07-01T07:07:07Z`.
    ///
    /// Identical to [`Self::to_rfc3339`] except that UTC renders as
    /// `Z` — the exact form the `time` crate's RFC 3339 formatter
    /// produced before this module existed, so news-sitemap output
    /// stays byte-identical across the migration.
    #[must_use]
...
/// every attempted format so call-site `log::warn!` lines say
/// exactly what was tried and on which value (spec A4).
...
/// `true` for Gregorian leap years (divisible by 4, except
...
///
/// Deterministic and locale-independent: month names come from
/// fixed English tables and no system time is read.
///
/// # Examples
///
/// ```rust
/// use staticdatagen::utilities::dates::parse_flexible_date;
///
/// let dt = parse_flexible_date("Wed, 01 Jul 2026 07:07:07 +0000")
///     .expect("RFC 2822 parses");
/// assert_eq!(dt.to_w3c_date(), "2026-07-01T07:07:07Z");
///
/// let dt =
///     parse_flexible_date("July 1, 2026").expect("long form parses");
/// assert_eq!(dt.to_rfc2822(), "Wed, 01 Jul 2026 00:00:00 +0000");
///
/// let dt =
///     parse_flexible_date("2026-07-01").expect("ISO date parses");
...
fn month_from_name(name: &str) -> Option<u8> {
    ...
    MONTH_FULL
        ...
        .position(|full| {
            *full == lower
                || (lower.len() == 3 && full.starts_with(&lower))
        })
        .map(|idx| idx as u8 + 1)
}
...
/// feeds routinely carry the wrong weekday and the previous
/// generator-local parser already tolerated that.
...
fn parse_long_form(input: &str) -> Option<FlexibleDate> {
    ...
    let (month, day_token) =
        month_from_name(tokens[0]).map(|m| (m, tokens[1])).or_else(
            || month_from_name(tokens[1]).map(|m| (m, tokens[0])),
        )?;
    if day_token.len() > 2 {
        return None;
    }
...
mod tests {
    ...
    fn rfc2822_wrong_weekday_is_tolerated() {
        ...
        // legacy parser ignored the weekday — so do we.
    ...
    fn rfc2822_positive_hhmm_offset() {
        ...
        // Non-UTC offsets keep the numeric form in W3C output.
        assert_eq!(dt.to_w3c_date(), "2026-12-25T18:30:45+05:30");
    ...
    fn rfc2822_named_zones() {
        let gmt =
            parse_flexible_date("11 Apr 2026 06:06:06 GMT").unwrap();
        assert_eq!(gmt.offset_minutes, 0);
        let est =
            parse_flexible_date("11 Apr 2026 06:06:06 EST").unwrap();
        assert_eq!(est.offset_minutes, -300);
        assert!(
            parse_flexible_date("11 Apr 2026 06:06:06 XYZ").is_err()
        );
    }
    ...
    fn long_form_month_first_single_digit_day() {
        ...
        assert_eq!(dt.to_rfc2822(), "Wed, 01 Jul 2026 00:00:00 +0000");
        assert_eq!(dt.to_w3c_date(), "2026-07-01T00:00:00Z");
    }
    ...
    fn iso_datetime_with_colon_offset() {
        let dt =
            parse_flexible_date("2026-07-01T07:07:07+05:30").unwrap();
        assert_eq!(dt.offset_minutes, 330);
        let dt =
            parse_flexible_date("2026-07-01T07:07:07-07:00").unwrap();
        assert_eq!(dt.offset_minutes, -420);
    }
    ...
    fn iso_datetime_with_compact_offset() {
        let dt =
            parse_flexible_date("2026-07-01T07:07:07+0530").unwrap();
        assert_eq!(dt.offset_minutes, 330);
    }
    ...
    fn iso_datetime_fractional_seconds_ignored() {
        let dt =
            parse_flexible_date("2026-07-01T07:07:07.123Z").unwrap();
        assert_eq!(dt.second, 7);
        assert!(
            parse_flexible_date("2026-07-01T07:07:07.abcZ").is_err()
        );
    }
    ...
    fn iso_rejects_invalid_components() {
        ...
        assert!(
            parse_flexible_date("2026-07-01T07:07:07+2500").is_err()
        );
    }
    ...
    fn leap_day_validation() {
        ...
        assert!(parse_flexible_date("Thu, 29 Feb 2024 12:00:00 +0000")
            .is_ok());
    ...
    fn error_names_all_attempted_formats() {
        ...
        assert!(
            msg.contains("RFC 2822"),
            "message names RFC 2822: {msg}"
        );
        assert!(
            msg.contains("long form"),
            "message names long form: {msg}"
        );
        assert!(
    ...
    fn weekday_computation_across_calendar() {
        ...
        for (iso, weekday) in cases {
            ...
            assert!(
                dt.to_rfc2822().starts_with(weekday),
                "{iso} should be {weekday}, got {}",
                dt.to_rfc2822()
            );
        }
    }
    ...
    // ---------------------------------------------------------------
    // Property tests (plan §2 item 1.4: dates 1990–2100 round-trip
    // through all three spec formats)
    // ---------------------------------------------------------------
    ...
    mod proptests {
        ...
        proptest! {
            #[test]
            fn round_trips_all_three_spec_formats(
                ...
                let day =
                    day_seed.min(days_in_month(year, month));

                // RFC 2822 round-trip.
                let rfc2822 = FlexibleDate {
                    year, month, day, hour, minute, second,
                    offset_minutes: 0,
                    format: DateFormat::Rfc2822,
                }
                .to_rfc2822();
                let dt = parse_flexible_date(&rfc2822)
                    .expect("generated RFC 2822 parses");
                prop_assert_eq!(
                ...
                // Long-form round-trip (date precision only).
                let long_form = format!(
                    "{} {}, {}",
                    MONTH_FULL[usize::from(month - 1)],
                    day,
                    year
                );
                let dt = parse_flexible_date(&long_form)
                    .expect("generated long form parses");
                prop_assert_eq!(
                    (dt.year, dt.month, dt.day),
                    (year, month, day)
                );

                // ISO 8601 date round-trip.
                let iso = format!(
                    "{year:04}-{month:02}-{day:02}"
                );
                let dt = parse_flexible_date(&iso)
                    .expect("generated ISO date parses");
                prop_assert_eq!(dt.to_iso_date(), iso);
            }

            #[test]
            fn parse_never_panics(s in ".*") {
                let _ = parse_flexible_date(&s);
            }
        }
    }
}
